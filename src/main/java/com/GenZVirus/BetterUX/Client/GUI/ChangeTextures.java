...
public class ChangeTextures extends Screen {
	...
	protected void init() {
		...
		Squared = new Button(mc.getMainWindow().getScaledWidth() / 2 + 5, mc.getMainWindow().getScaledHeight() / 2 + 25, 100, 20, "Squared", (x) -> {
			resetButtons();
		...
		Improved = new Button(mc.getMainWindow().getScaledWidth() / 2 - 105, mc.getMainWindow().getScaledHeight() / 2 + 25, 100, 20, "Improved", (x) -> {
			resetButtons();
