...
//				mc.fontRenderer.drawString(new MatrixStack(), rightShieldDamageReductionText, rightShieldPosX + 17 - mc.fontRenderer.getStringWidth(rightShieldDamageReductionText) / 2, rightShieldPosY + 10, 0xFFFFFFFF);
		...
		AbstractGui.blit(leftShieldPosX * 5, leftShieldPosY * 5, 0, 0, 0, 64, 64, 64, 64);
		...
		AbstractGui.blit(rightShieldPosX * 5, rightShieldPosY * 5, 0, 0, 0, 64, 64, 64, 64);
			...
			for (EffectInstance effectinstance : Ordering.natural().reverse().sortedCopy(collection)) {
				...
				if (effectinstance.isShowIcon()) {
					...
					if (effectinstance.isAmbient()) {
						AbstractGui.blit(k, l, -90, 165, 166, 24, 24, 256, 256);
					} else {
						AbstractGui.blit(k, l, -90, 141, 166, 24, 24, 256, 256);
					...
					list.add(() -> {
						...
						AbstractGui.blit(j1 + 3, k1 + 3, -90, 18, 18, textureatlassprite);
					});
					effectinstance.renderHUDEffect(mc.currentScreen, k, l, (float) -90, f);
				}
			}
